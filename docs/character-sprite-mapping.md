...
`tools/art_pipeline/character_sheet_map.json` — one ordered JSON table of the 13 rows plus the grid
...
| columns | **8** | max 8 frames per animation (walk uses all 8; others use 6) |
| rows | **13** | one row per animation, top-to-bottom in map order |
| sheet size | **2048 × 3328 px** | 8 columns wide × 13 rows tall |
...
- **Blank-cell padding.** Rows with fewer than 8 frames are padded to the right with
  **fully transparent** cells. The tool detects these by zero alpha and skips them, so the actual
  frame count of an animation is its number of non-blank cells — never a hard 8.
...
| 1 | `walk` | yes | 8 | walk cycle |
| 2 | `run` | yes | 6 | run cycle |
| 3 | `jump` | no | 3 | jump rise (one-shot) |
| 4 | `fall` | yes | 3 | falling/airborne |
| 5 | `land` | no | 2 | landing recovery (one-shot) |
| 6 | `attack_1` | no | 4 | basic attack, chain 1 |
| 7 | `attack_2` | no | 4 | basic attack, chain 2 |
| 8 | `attack_3` | no | 6 | basic attack, chain 3 (finisher) |
| 9 | `attack_air` | no | 4 | air attack |
| 10 | `hit` | no | 3 | hurt reaction (one-shot) |
| 11 | `dodge` | no | 6 | dodge/dash |
| 12 | `death` | no | 6 | death (one-shot, hold last frame) |
...
1. Create one **tag** per animation row, named exactly as the map (`idle`, `walk`, `run`, `jump`,
   …).
...
It emits `docs/templates/character-sheet-template.png` — a 2048 × 3328 grid with faint cell lines
and one row label per animation (`name (frames, loop|once)`). Any change to the JSON map is reflected
by re-running this command.

## 7. Hand-cleaned frames pipeline

When the human has exported clean per-action frame folders (one directory of PNGs per animation,
e.g. `walk/frame_000.png`), the official pipeline is
`tools/art_pipeline/compose_from_user_frames.py`. It NEAREST-resizes any non-256px frame to 256×256,
writes the flat project frames and the mirrored `frames_left` folders, composes both sheets at
8 × 13 cells, and emits both `SpriteFrames` `.tres` with the UIDs pinned. See
`tools/art_pipeline/README.md` for flags.

## 8. Related documents

- `tools/art_pipeline/character_sheet_map.json` — single editable source (this doc mirrors it).
- `tools/art_pipeline/compose_from_user_frames.py` — official pipeline for hand-cleaned frame folders.
